...
<p>Your choose Guess Point (5 Cards)</p>
...
if (!empty($_POST["card_8"])) {
	...
	header("location: guess_point8.php");
		...
		echo "<script>alert('You Level is no enough, the target level is level 2!')</script>";
		$step = 0;
		$init = 0;
		$random_flag = 1;
...
$app_name = "NYHC - Guess Point (5 Cards)";
...
$init_bet = (empty($_COOKIE["pref_init_bet"]))	? 10 : $_COOKIE["pref_init_bet"] ;
    ...
    if (!empty ($_POST["point38_48"])){
		if ($step == 0) {	
			...
			if ($init == 0) {	
				...
				switch ($result2) {
					case 5: 
                        $msg = "You Win! <br> You point is $total_point!";
                        $balance += ($bet * 20); 
                        $userPoint = $balance;
						$exp_plus = ($bet * 0.01) + $userExp;
						$userExp = $exp_plus;
...
case 0:
	...
	echo " $page_header
		...
		<table border=1 cellspacing=1 cellpadding=1 align=center id = 'game_table'>
			...
			<tr align = center>
     			<td><input type=\"submit\" name= \"point05_15\" value=\"05-15\" id = 'game_button1'></td>
				<td><input type=\"submit\" name= \"point16_26\" value=\"16-26\" id = 'game_button1'></td>
				<td><input type=\"submit\" name= \"point27_37\" value=\"27-37\" id = 'game_button1'></td>
    			<td><input type=\"submit\" name= \"point38_48\" value=\"38-48\" id = 'game_button1'></td>
			    <td><input type=\"submit\" name= \"point49_60\" value=\"49-60\" id = 'game_button1'></td>
			...
			<tr align=center>
				...
				<input type=\"hidden\" name=\"c5d\" value= " . $cards[5][2] . "> </tr>
		</table>
		</form>
		</body>";
	break;
...
case 4:
	...
	echo "$page_header
		...
		<table border=1 cellspacing=1 cellpadding=1 align=center>
			...
			<tr><td align = center colspan = 6><b>About Guess Point (5 Cards)</b></td></tr>

			<tr><td align = center colspan = 5>
			<table>
			<tr><td align = center><b>**Winning Reward**</b></td></tr>
			<tr><td>Between Point</td><td align = right>Point</td></tr>
			<tr><td>49-60</td><td align = right>30</td></tr>
			<tr><td>38-48</td><td align = right>20</td></tr>
			<tr><td>27-37</td><td align = right>10</td></tr>
			<tr><td>16-26</td><td align = right>20</td></tr>
			<tr><td>5-15</td><td align = right>30</td></tr>
...
function get_result() {
	...
	if($result2 == 0){
        if(($total_point > 48) and ($total_point < 61)){
			$result2 = 1;
        }
    }
    
    if($result2 == 0){
        if(($total_point > 37) and ($total_point < 49)){
		    $result2 = 2;
        }
    }

    if($result2 == 0){
        if(($total_point == 27) or ($total_point == 28) or ($total_point == 29) or ($total_point == 30) or 
		   ($total_point == 31) or ($total_point == 32) or ($total_point == 33) or ($total_point == 34) or 
		   ($total_point == 35) or ($total_point == 36) or ($total_point == 37)){
		    $result2 = 3;
        }
    }

    if($result2 == 0){
        if(($total_point > 15) and ($total_point < 27)){
		    $result2 = 4;
        }
    }

    if($result2 == 0){
        if(($total_point > 4) and ($total_point < 16)){
		    $result2 = 5;
        }
    }
...
function get_result2() {
    ...
    if($result3 == 0){
        if(($total_point == 5) or ($total_point == 7) or ($total_point == 9) or ($total_point == 11) or
		   ($total_point == 13) or ($total_point == 15) or ($total_point == 17) or ($total_point == 19) or
		   ...
		   ($total_point == 53) or ($total_point == 55) or ($total_point == 57) or ($total_point == 59)){
		    $result3 = 6;
        }
    }
